...
#draw plot
ggplot(director_CI,aes(x=mean_rating,y=director,colour=director))+
  #add labs
  ...
  geom_point(size=5)+
  #create CI bar
  geom_errorbar(aes(xmin=rating_low,xmax=rating_high),width=0.1,size=2)+
  #add text beside points
  geom_text(aes(label=round(mean_rating,digits=2)),vjust = 0, nudge_y = 0.05,colour="black",size=6)+
  geom_text(aes(label =round(rating_low,digits=2), x = rating_low),vjust = 0, nudge_y = 0.05,colour="black",size=5) +
  geom_text(aes(label =round(rating_high,digits=2), x = rating_high),vjust = 0, nudge_y = 0.05,colour="black",size=5) +
  #add intersection bar
  geom_rect(
    fill="grey",alpha=0.5,colour="grey",
    aes(xmin=as.numeric(director_CI$rating_low[director=="Steven Spielberg"]),
    xmax=as.numeric(director_CI$rating_high[director=="Tim Burton"]),
    ymin=0,
    ymax=Inf))+

  #set scale
  scale_x_continuous(limits=c(6.5,8),breaks = seq(6.5, 8, by = 0.5))+
...
Hypothesis testing
```{r}
diff_in_means=director_CI$mean_rating[1]-director_CI$mean_rating[2]
set.seed(1234)
director_hypothesis<-movies %>%
  filter(director %in% c("Steven Spielberg","Tim Burton"))%>% 
...
director_hypothesis %>% visualize()
director_hypothesis %>%get_pvalue(obs_stat=diff_in_means,direction="both")
```
The p value is smaller than 5%, so we reject the null hypothesis. The rating scores of the two directors are different.
...
We have 1004 missing observations of `weight`. The distribution of `weight` is right skewed.
...
95% confidence interval for the population proportion of high schools that are *NOT* active 3 or more days per week is 0.317 to 0.345
...
yrbss %>% filter(!is.na(physical_3plus)) %>% 
  ...
  summarise(count = n()) %>% 
  mutate(prop= count/sum(count),
         se = sqrt(prop*(1-prop)/count),  #calculate stnd error
         t_critical = qt(0.975, count-1), #calculate t critical for 95% CI
         CI_low = prop - t_critical*se,   #calculate low end for 95% CI
         CI_high = prop + t_critical*se)  #calculate high end for 95% CI

```

The median `weight` of people who are physically active for at least 3 days a week is larger than who are not, and people who are physically active for at least 3 days a week have less extreme `weight`.
I expected the median `weight` people who are physically active for at least 3 days a week is less since they tend to consume more energy.
...
null hypotheses: mean weights are the same for those who exercise at least 3 times a week and those who don’t.
alternative hypotheses: mean weights are different for those who exercise at least 3 times a week and those who don’t.
...
>>>>>>> aa01c3b1b7f3eed475d8cce54c470f3ec16d8d8b
=======
---
title: "Session 6: Homework 3"
author: "Group 29"
...
```{r yield_curve_challenge, echo=FALSE, out.width="50%"}
knitr::include_graphics(here::here("images", "yield_curve_challenge.png"), error = FALSE)
```
...
maturity <- c('3m', '6m', '1y', '2y','3y','5y','7y','10y','20y','30y')
maturity_order <- c('3m', '6m', '1y', '2y','3y','5y','7y','10y','20y','30y')
...
knitr::include_graphics(here::here("images", "yield_curve1.png"), error = FALSE)
```
```{r yield_curve_g_1, out.width="100%"}
# myvars <- factor(myvars, levels = myvars)


yield_curve <- yield_curve %>% 
  mutate(var = factor(var, order = TRUE, levels = myvars))

yield_1 <- yield_curve %>%
  ggplot(aes(x = date, y = price, color = as.factor(var))) +
  geom_line() +
  facet_wrap(~var, nrow = 5)+
  scale_x_date(limits = as.Date(c('1960-01-01','2020-01-01')))+
  scale_color_manual(values = c("red", "#ebc77f", "#acae19", "#a0db84", "#31cb96","#93e4e6", "#82d8fa", "#ccc9ff", "#f1a8f8", "#ff90cf"))+
...
knitr::include_graphics(here::here("images", "yield_curve2.png"), error = FALSE)
```
```{r yield_curve_g_2, out.width="100%"}
# myvars <- factor(myvars, levels = myvars)


yield_curve <- yield_curve %>% 
  mutate(maturity = factor(maturity, order = TRUE, levels = maturity_order))

yield_2 <- yield_curve %>%
  filter(year %in% c(1999: 2020)) %>%
  ggplot(aes(x = maturity, y = price, color = as.factor(year),group = date)) +
  geom_line() +
...
knitr::include_graphics(here::here("images", "yield_curve3.png"), error = FALSE)
```
```{r yield_curve_g_3, out.width="100%"}
# myvars <- factor(myvars, levels = myvars)
# Turn  FRED codes to human readable variables
...
yield_3 <- yield_curve %>%
  ...
  labs(title = "Yields 3 month and 10 year U.S. Treasury rates since 1999 ",
       x="",
       y = "%",
       caption="Source: St. Louis Federal Reserve Economic Database (FRED)",
       color="") +
...
order_list <- c("Gross capital formation", "Exports", "Government expenditure", "Household expenditure", "Imports")
```
...
ggplot(plot1, aes(x=years, y=value_bn, colour = reorder(IndicatorName, order_list)))+
...
>>>>>>> 01ed55d12d957e6e190a75e2f6bb1c980ba91467
