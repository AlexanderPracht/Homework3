...
```{r}
plot4 <- yield_curve%>%
  filter(var == c("3-Month Treasury Bill", "10-Year Treasury Rate"))%>%
  mutate(var = case_when(
        var %in% "3-Month Treasury Bill" ~ "three",
        var %in% "10-Year Treasury Rate" ~ "ten"))%>%
  pivot_wider(names_from = var, values_from = price)%>%
  group_by(date)

%>%
  summarise("three" = sum(three, na.rm = TRUE), "ten" = sum(ten,na.rm = TRUE))
...
order_l <- factor(order_list, levels = order_list) 

ggplot(plot1, aes(x=years, y=value_bn, colour = order(IndicatorName, order_l)))+
...
ggplot(comparison, aes(x= years))+
  ...
  geom_line(aes(y=difference, group =1))+
  facet_wrap(~Country)+
  scale_x_discrete(breaks = seq(1970, 2017, by = 10))+
